...
describe "Happiness" do
  before do
    @pet = Pet.create(name: "Spot", type: "Dog", gender: "Male")
  end
...
describe "Hunger" do
  before do
    @pet = Cat.create(name:"Sylvie", gender:"female")
    @time = Time.now
    @pet.pet_hunger.last_interaction = @time - (60*60*8)
...
describe "Boredom" do
  before do
    @pet = Cat.create(name:"Sylvie", gender:"female")
    @time = Time.now
    @pet.pet_boredom.last_interaction = @time - (60*60*8)
...
describe "Tiredness" do
  before do
    @pet = Cat.create(name:"Sylvie", gender:"female")
    @time = Time.now
    @pet.pet_tiredness.last_interaction = @time - (60*60*6)
    #last slept 6 hours ago
  end
